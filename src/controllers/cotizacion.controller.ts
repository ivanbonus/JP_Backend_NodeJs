 ...
 */
async function generarPdfBuffer(data: any): Promise<Buffer> {
    ...
    try {
        ...
        if (foundPath) {
            const logoData = fs.readFileSync(foundPath);
    ...
    const safeReplace = (content: string, key: string, value: string) => {
...
export const getCotizaciones = async (req: Request, res: Response) => {
  try {
    const cotizaciones = await prisma.cotizacion.findMany({ orderBy: { fecha: 'desc' } });
...
export const updateCotizacionStatus = async (req: Request, res: Response) => {
  try {
      ...
      if (!estado) { res.status(400).json({ error: 'Falta el estado.' }); return; }
      const updated = await prisma.cotizacion.update({ where: { id: parseInt(id as string) }, data: { estado } });
      res.json(updated);
  } catch (error) {
...
export const responderCotizacion = async (req: Request, res: Response) => {
  try {
    ...
    const cotizacionDB = await prisma.cotizacion.findUnique({ where: { id: parseInt(id as string) } });
    if (!cotizacionDB) { res.status(404).json({ error: 'Cotización no encontrada.' }); return; }
    ...
    try {
        await enviarCotizacionEmail(
            ...
            `<div style="font-family: Arial, sans-serif;"><h2>Hola, ${cotizacionDB.nombre}.</h2><p>Adjuntamos la cotización formal.</p></div>`,
            Buffer.from(pdfBuffer),
            `Cotizacion_${cotNum}.pdf`
        );
    } catch (e) {
